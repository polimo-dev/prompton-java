...
| `use_case.json` | Use-case document + use case (+ prompt name) → model, params, prompt version, filled messages | 3 documents, 15 cases |
...
`variables` is present — fill the resulting prompt. This is precisely what `POST /api/v1/use-cases/{key}/prompt`
does on the server.
...
* If the use-case document references a prompt version or model id it does not contain, selection
